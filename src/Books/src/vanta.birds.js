...
const ATTACH_CLASSES = function (THREE) {
  ...
  THREE.BirdGeometry = function (options) {
    ...
    for (let f = 0; f < BIRDS; f++) {
      ...
      verts_push(0, 0, -30 * s, 0, 4 * s, -30 * s, 0, 0, 30 * s,
        0, 0, 30 * s, 0, 4 * s, 30 * s, 0, 4 * s, -30 * s) // Body
      verts_push(0, 0, -30 * s, -wingSpan * s, 0, -30 * s, 0, 0, 30 * s,
        0, 0, 30 * s, -wingSpan * s, 0, 30 * s, -wingSpan * s, 0, -30 * s) // Left Wing
      verts_push(0, 0, 30 * s, wingSpan * s, 0, 30 * s, 0, 0, -30 * s,
        0, 0, -30 * s, wingSpan * s, 0, -30 * s, wingSpan * s, 0, 30 * s) // Right Wing
    }

    const colorCache = {}

    for (v = 0; v < triangles * 6; v++) {
      const i = ~~(v / 6)
      const x = (i % WIDTH) / WIDTH
      const y = ~~(i / WIDTH) / WIDTH
      const order = ~~(v / 18) / BIRDS
...
void main() {
  ...
  if ( birdVertex == 7.0 || birdVertex == 10.0 || birdVertex == 11.0
    || birdVertex == 13.0 || birdVertex == 16.0 || birdVertex == 17.0) {
    // flap wings
...
class Birds extends VantaBase {
  static initClass() {
    this.prototype.defaultOptions = {
      ...
      backgroundColor: 0xFFFFFF, // 0x202428
      color1: 0xFF7875, // 0xf50000 # 0xfa9898
      color2: 0x91D5FF, // 0xcfcf1d # 0x8c4646
      colorMode: 'lerp',
      birdSize: 1,
      wingSpan: 40,
      speedLimit: 3,
  ...
  initGpgpuBirds() {
    ...
    this.birdUniforms = {
      color: { value: new THREE.Color(0xff7875) },
  ...
  onInit() {
    ...
    this.fog = new THREE.Fog(0xFF7875, 100, 1000)
