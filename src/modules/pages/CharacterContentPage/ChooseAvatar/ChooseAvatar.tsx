...
function ChooseAvatar(props: ChooseAvatarProps): React.ReactElement {
  ...
  const [selectedAvatar, setSelectedAvatar] = useState<string>(user.selectedAvatar);
        ...
        <Col span = {24} className="choose-avatar-col">
          {Array.from(avatarUrls.entries()).map((entry =>         
            <Card hoverable
              ...
              className={ selectedAvatar === entry[0] ? "avatar-card-selected" : "avatar-card"}
              onClick={() => {setSelectedAvatar(entry[0])}}
              key={entry[0]}
            >
              <Meta title={entry[0]}/>
            </Card>
